"""FastAPI service for 12-month hospitalization risk prediction."""
...
MODEL_PATH = Path(
    os.getenv(
        "MODEL_PATH",
        Path(__file__).resolve().parents[1] / "model" / "modelo_riesgo_hospitalizacion_v1.pkl",
    )
)
...
app = FastAPI(
    title="Hospitalization Risk Predictor",
    description="Predicts 12-month hospitalization risk from clinical/demographic features.",
    version="0.1.0",
    lifespan=lifespan,
)
...
def predict(patient: PatientFeatures) -> PredictionResponse:
    ...
    return PredictionResponse(
        probabilidad_hospitalizacion_12m=proba,
